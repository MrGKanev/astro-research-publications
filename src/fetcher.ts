...
import type { Publication, CoAuthor, CitationStats, ScholarData } from './types.js';
...
function parsePublications($: cheerio.CheerioAPI): Publication[] {
  ...
  $('#gsc_a_b .gsc_a_tr').each((_, row) => {
    ...
    publications.push({ id, title, authors, venue, year, citations, scholarUrl, citationsUrl });
  });
  return publications;
}
